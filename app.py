...
import json, glob, os
from grok_chat import get_grok_response, save_message

# API endpoint to get a list of all available chats.
# It looks for all the files in the directory that end with `_chat.json`
async def get_chats(request):
    chat_files = glob.glob('*_chat.json')
    chat_names = [os.path.splitext(os.path.basename(f))[0].replace('_chat', '') for f in chat_files]
    return web.json_response(chat_names)

# API endpoint to get the chat history for a specific chat.
# It reads the corresponding json file and returns the content.
...
async def get_chat_history(request):
    ...
    for item in history:
        ...
        transformed_history.append({'role': 'assistant', 'content': item['response']})
...
async def add_message(request):
    ...
    save_message(chat_name, new_message)

    return web.json_response({'role': 'assistant', 'content': new_message['response']})
